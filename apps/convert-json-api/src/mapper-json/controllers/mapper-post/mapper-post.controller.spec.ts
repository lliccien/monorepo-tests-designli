...
describe('MapperGetController', () => {
  ...
  const originalRecord: OriginalRecord = {
    Records: [
      {
        eventVersion: '1.0',
        ses: {
  ...
  beforeEach(async () => {
    mockMapperService = {
      mappedRecord: jest
        .fn()
        .mockImplementation((originalRecord: OriginalRecord) => {
          // Aquí puedes simular una respuesta específica basada en el record proporcionado
          return transformedRecord;
        }),
    };
    const module: TestingModule = await Test.createTestingModule({
      controllers: [MapperPostController],
      providers: [{ provide: MapperService, useValue: mockMapperService }],
    }).compile();
  ...
  it('should transform original record to transformed record', async () => {
    const result = await controller.transform(originalRecord);

    expect(result).toEqual(transformedRecord);
    expect(mockMapperService.mappedRecord).toHaveBeenCalledWith(originalRecord);
  });
});
